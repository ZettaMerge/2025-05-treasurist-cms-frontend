...
import {AgentDTO, FundPlanDraftDTO} from '@model';
...
@Component({
  selector: 'fe-switching-orders',
  templateUrl: './fe-switching-orders.component.html',
  styleUrls: ['./fe-switching-orders.component.scss']
})
export class FeSwitchingOrdersComponent extends BaseFeatureGridComponent<PropType, any> implements OnInit {
  ...
  draftType = ['SUBSCRIPTION_ORDER', 'REDEMPTION_ORDER', 'SWITCHING_ORDER'];
  draftTypeList = [
    {id: 1, name: 'BUY', value: 'SUBSCRIPTION_ORDER'},
    {id: 2, name: 'SELL', value: 'REDEMPTION_ORDER'},
    {id: 3, name: 'SWITCH', value: 'SWITCHING_ORDER'},
  ];
  ...
  detailInfo(item) {
    const url = `agent/customer-service/switching-order/${item.id}`;
  ...
  protected getData() {
    ...
    this.dataSub = this.agentService.customerFundPlanDraftListGet$(
      ...
      this.sortData,
      this.direction,
      this.filter?.draftType ? this.filter?.draftType?.value : this.draftType,
