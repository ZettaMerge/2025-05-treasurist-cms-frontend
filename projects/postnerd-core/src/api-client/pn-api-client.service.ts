...
import { PnApiClientConfig, defaultConfig } from './pn-api-client.config';
import { ApiError, ApiErrorHandler } from './pn-api-error.model';
...
export class PnApiClientService {
  ...
  public request(
    ...
    if (httpMethod.toLowerCase() === 'get') {
      ...
      return this.patch(url, options.body, options.params, isPublic, headerResponse);
  ...
  public patch(url: string, body: any, params?: Params, isPublic?: boolean, headerResponse?: HeaderResponse): Observable<any> {
    return this.requestHelper('Patch', url, { body, params }, isPublic, headerResponse).pipe(
      this.globalErrorHandler(),
    );
  }
  ...
  private globalErrorHandler(): OperatorFunction<any, any> {
    return catchError((err: ApiError) => {
      ...
      const alertTimeout = window.setTimeout(() => {
        ...
        if (err.status === 401 && (errorHandler.message === 'Unauthorized')) {
  ...
  private execute(method: string, url: string, options: HttpClientRequestOptions, isPublic?: boolean): Observable<Response> {
    if (!isPublic) {
      ...
      if (token) {
        options.headers = options.headers.set('JWTAuthorization', `${token}`);
      }
    }
  ...
  private errorHandler(err): ApiErrorHandler {
    let message: string;
    try {
